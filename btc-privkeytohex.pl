...
# privkeytohex.pl
# ===============
# Convert Bitcoin WIF-compressed or Bitcoin WIF-uncompressed private key into 64 character hex string
# representing the binary key.
# Usage :
# see `perl privkeytohex.pl -h'
#
# Testing
# =======
...
if (@ARGV == 0 || $ARGV[0] eq "-h") {
	print(	"-----------------------------------------------------\n" .
			"privkeytohex.pl Bitcoin WIF/Hex private key converter\n" .
			"-----------------------------------------------------\n" .
			BLANKLINE .
			"Usage:\n" .
			BLANKLINE .
			"To convert Bitcoin WIF-compressed or WIF-uncompressed private key to 64\n" .
			"character hex string representing the binary key :-\n" .
			"perl privkeytohex.pl <key>\n" .
			BLANKLINE .
			"To take input from a file(s) :-\n" .
			"perl privkeytohex.pl -f <filename1> <filename2> ... \n" .
			...
			"<other program> | perl privkeytohex.pl -f\n"
	);
	...
	if ( @ARGV != 0 ) {
		print(STDERR "Invalid parameters : run 'perl privkeytohex.pl -h' for usage information.\n");
		exit 1;
	}
	$priv_key = $param1;
	$priv_key_bin = &get_priv_key_bin($priv_key);
