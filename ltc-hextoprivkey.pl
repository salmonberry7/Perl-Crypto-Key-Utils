...
###############################################################################################################
# NOTE: this script was created for Litecoin by adapting the corresponding Bitcoin script hextoprivkey.pl 
# See Litecoin Note.txt file for description of the changes.
###############################################################################################################

# litecoinhextoprivkey.pl
# =======================
# Convert 64 character hex string into Litecoin WIF-compressed or Litecoin WIF-uncompressed private key
# Usage :
# see `perl litecoinhextoprivkey.pl -h'
#
# Testing
# =======
...
# The returned private key always starts with a 'T' (compressed key) or a '6' (uncompressed key).
# Assumes a 32 byte binary string is passed as input.
sub get_private_key {
	my $priv_key_bin = $_[0];
	my $is_compressed_priv_key = $_[1];

	my $private_key_encode = "\xB0$priv_key_bin";
...
if (@ARGV == 0 || $ARGV[0] eq "-h") {
	print(	"--------------------------------------------------------------\n" .
			"litecoinhextoprivkey.pl Litecoin Hex/WIF private key converter\n" .
			"--------------------------------------------------------------\n" .
			BLANKLINE .
			"Usage:\n" .
			BLANKLINE .
			"To convert 64 character hex string <hex> to Litecoin WIF-compressed private key :-\n" . 
			"perl litecoinhextoprivkey.pl --compressed|-c <hex>\n" .
			BLANKLINE .
			"To convert 64 character hex string <hex> to Litecoin WIF-uncompressed private key :-\n" . 
			"perl litecoinhextoprivkey.pl --uncompressed|-u <hex>\n" .
			BLANKLINE .
			"The hex string should not have any '0x' prefix.\n" .
			BLANKLINE .
			"To take input from a file(s) :-\n" .
			"perl litecoinhextoprivkey.pl -f <filename1> <filename2> ... \n" .
			...
			"<other program> | perl litecoinhextoprivkey.pl -f\n" .
			BLANKLINE .
	...
	if ( @ARGV != 1 ) {
		print(STDERR "Invalid parameters : run 'perl litecoinhextoprivkey.pl -h' for usage information.\n");
		exit 1;
	}
